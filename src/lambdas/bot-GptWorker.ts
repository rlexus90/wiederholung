...
export const GptWorker = async () => {
  const client = new LambdaClient({});
  const code = await readFile(
    path.resolve(__dirname, '../../dist', 'worker.zip')
  );
  const ZipFile = new Uint8Array(code);

  const FunctionName = NAMES.worker;
  ...
  try {
    ...
    console.log('Bot-worker updated');
  } catch {
    try {
      await client.send(create);
      console.log('Bot-worker created');
    } catch (err) {
      console.log(err);
    }
  }
